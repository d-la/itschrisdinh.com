<header class="banner relative header container min-h-[100px] md:min-h-[150px] flex justify-between items-center z-50">
  <a href="{{ home_url('/') }}" class="header__website-logo brand component site-name-base text-3xl md:text-4xl lg:text-5xl text-white mt-[10px]">
      Chris D.
  </a>
      ...
      <nav class="w-full flex flex-col justify-end items-end text-white lg:justify-start lg:items-start z-20" aria-label="Primary Navigation">
          ...
          <ul class="header-socials flex flex-row gap-5 items-start">
            ...
            @if (!empty($emailAddress))
              <li>
                <a href="mailto:{{ $emailAddress }}" class="text-2xl md:text-4xl copy-base transition-transform duration-300 ease-in-out group" aria-label="Email Chris Dinh">
