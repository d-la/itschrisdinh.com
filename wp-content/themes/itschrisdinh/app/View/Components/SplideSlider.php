...
class SplideSlider extends Component {
    ...
    public function __construct( $sliderSettingsAcfName, $slideViewTemplatePath, $slideViewTemplateData, $acfPostId = false ) {
        ...
        $this->setCustomPaginationViewSettings( $this->sliderSettings['custom_pagination'] );
    }
    ...
    private function formatCustomSliderSettings() {
        ...
        foreach ( $this->sliderSettings['custom_settings'] as $breakpoint => $settings ) {
            if ( !empty($settings) ) {
    ...
    {
        return $this->view('components.splide-slider');
    }
}
